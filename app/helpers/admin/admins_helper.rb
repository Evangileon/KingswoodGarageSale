module Admin
  ...
  def status_tag(status)
    content_tag :span, t("orders.statuses.#{status}"), :class => "status-tag #{status}"
  end
    ...
    case type
      when 'boolean'
        String.new.tap do |s|
          ...
          s << label_tag("settings_#{field}_true", t("settings.options.#{field}.affirmative", :default => 'Yes'))
          s << radio_button_tag("settings[#{field}]", 'false', value == false, :id => "settings_#{field}_false")
          s << label_tag("settings_#{field}_false", t("settings.options.#{field}.negative", :default => 'No'))
          s << "</div>"
        end.html_safe
      else
